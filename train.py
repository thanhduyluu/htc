...
from models.hill.optim import ScheduledOptim, Adam
import os
from eval import evaluate
from models.hill.models import StructureContrast
...
class BertDataset(Dataset):
    def __init__(self, max_token=512, device='cpu', pad_idx=0, data_path=None):
        ...
        self.label_tokens = data_utils.load_indexed_dataset(
            data_path + '/label', None, 'mmap'
        )
    ...
    def __getitem__(self, item):
        ...
        label_token = self.label_tokens[item].to(self.device)
        return {'data': data, 'label': labels, 'idx': item, 'label_token': label_token}
    ...
    def collate_fn(self, batch):
        ...
        if not isinstance(batch, list):
            return batch['data'], batch['label'], batch['idx'], batch['label_token']
        label = torch.stack([b['label'] for b in batch], dim=0)
        label_token = torch.stack([b['label_token'] for b in batch], dim=0)
        ...
        return data, label, idx, label_token
...
def params_and_time(config, train_set, test_set, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        model.train()
        for data, label, idx, label_token in train_set:
    ...
    model.eval()
    with torch.no_grad():
        for data, label, idx, label_token in test_set:
            padding_mask = data != tokenizer.pad_token_id
...
def run_train(config, train_set, dev_set):
    ...
    for epoch in range(config.train.epoch):
        ...
        loss = 0.0
        # -------------Train-----------------------------
        for index, (data, label, idx, label_token) in enumerate(train_set):
