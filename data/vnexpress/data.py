...
if __name__ == '__main__':
    model = 'vinai/phobert-base'  # For online.
    ...
    with open('wos_total.json', 'r') as f:
        for line in f.readlines():
            ...
            labels.append(line['doc_label'])
    set_labels = {"[PAD]", "[CLS]", "[SEP]", "[MASK]", "[UNK]", "O", "[NEXT]"}
    for i in labels:
        for j in i:
            set_labels.add(j)
    tokenizer.add_tokens(list(set_labels))
    for l in labels:
        label_sentences = "[NEXT]".join(i).strip().lower()
        label_token = tokenizer.encode(label_sentences.strip().lower(), truncation=True)
        labels_tokens.append(label_token)
